...
namespace SpecFlowIntegration.FileProcessingTest.Service
{
    public class CommonTestService
    {
        ...
        public ClientFileType GetClientFileType(string fileType, string fileGroupCode)
        {
            var cftRepo = new ClientFileTypeRepository();
            var candidates = cftRepo.GetClientFileTypeRecordByOrganization("Citi")
                .Where(cft => string.Equals(cft.FileType, fileType, StringComparison.InvariantCultureIgnoreCase)
                              && string.Equals(cft.FileGroupCode, fileGroupCode, StringComparison.InvariantCultureIgnoreCase)).ToList();

            if (candidates.Count == 0)
            {
                throw new Exception("Could not find " + fileType + " " + fileGroupCode + " generator CFT");
            }

            if (candidates.Count > 1)
            {
                throw new Exception("More than one CFT found for this generator");
            }
        ...
        {
            // due to issue with RPC call, the whole inventory for Citi is queried so it takes long time
            // consider moving all Citi to test_Client

            var timeout = DateTime.Now.Add(TimeSpan.FromSeconds(900));
